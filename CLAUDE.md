...
ソクラテス式のAI対話レッスン、ルーブリック採点、FSRS間隔反復、ロードマップ管理を提供する。
...
- **ロール分離**: tutor(対話)/ grader(5観点採点)/ cardGrader(軽量採点)/ variantGenerator(変形問題)の4プロンプトモジュールを分離。graderは構造化出力(Zod)のみ。
- **ガードレール**: (1) 学習者の試行前に模範解答を出さない (2)「答えを見る」はイベント記録+直後に変形問題を1問 (3) テーゼ・引用は curriculum.json 由来に限定(正典外の引用生成禁止)。
